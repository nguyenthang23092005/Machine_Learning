...
# Load pre-trained SVM model from joblib
...
if faces:
    for face in faces:
        ...
        # Flatten the face features for prediction
        ...
        # Calculate cosine similarity using the support vectors of the SVM model
        support_vectors = svm_model.support_vectors_  # Access the support vectors
        distances = cosine_similarity(face_features, support_vectors)  # Calculate cosine similarity

        # If the similarity is lower than the threshold, label as Unknown
        if np.max(distances) < similarity_threshold:
            svm_label = "Unknown"

        # Print the predicted label
        print(f'Predicted label: {svm_label}')  # Print prediction result
